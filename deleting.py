...
def delete_address(file_path):
    lines = read_lines(file_path)

    while True:
        n = input("Silinecek Mahalle: ")
        p = input("İl: ")
        d = input("İlçe/Belde (isteğe bağlı): ")
        c = input("Merkez tipi (örn: DISTRICT CENTER): ")

        target_line = format_address(n, p, d, c)

        normalized_lines = list(map(normalize_line, lines))
        if normalize_line(target_line) in normalized_lines:
            index = normalized_lines.index(normalize_line(target_line))
            deleted = lines.pop(index)
            write_lines(file_path, lines)
            print(f"Adres silindi: {deleted}")
            break
        else:
            print("Adres bulunamadı. Lütfen bilgileri kontrol edin ve tekrar deneyin.\n")

if __name__ == "__main__":
    delete_address("neighborhoods.txt")
